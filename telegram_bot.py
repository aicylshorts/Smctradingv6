...
class TelegramNotifier:
    ...
    def _format_alert(self, setup) -> str:
        ...
        # Force timestamp to WAT regardless of source timezone
        ts_wat = setup.timestamp
        if hasattr(ts_wat, 'tzinfo') and ts_wat.tzinfo is not None:
            ts_wat = ts_wat.astimezone(WAT)
        else:
            ts_wat = ts_wat.replace(tzinfo=timezone.utc).astimezone(WAT)
